...
| 備份 | ✅ 已設定至本專案 `.oab-control/backups`（本機 bootstrap backup） |
...
/home/nickhuang23/oab-agents-control/.oab-control/backups/  備份目標
```
...
PYTHONPATH=. python3 -m oab_control.cli backup config/backup-sources.json \
  --output /home/nickhuang23/oab-agents-control/.oab-control/backups \
  --attestation "local project-directory bootstrap backup" \
  --yes --json
```

> ⚠️ `.oab-control/backups` 是本專案目錄內的一般本機備份，**不是加密外部儲存**。
...
PYTHONPATH=. python3 -m oab_control.cli deploy config/catalog.yaml \
  ...
  --backup-output /home/nickhuang23/oab-agents-control/.oab-control/backups \
  --backup-attestation "local project-directory bootstrap backup" \
...
掛載 workspace 不等於 agent 知道它存在。agent 行程從家目錄啟動，
`config.toml` 完全沒提到 `/workspaces`——**沒有這一步，44 個掛載不會有人去看**。

chart 支援把 `AGENTS.md` 掛進工作目錄（同一份內容也掛成 `CLAUDE.md` 與
`GEMINI.md`，涵蓋不同 CLI 的慣例）。renderer 從 catalog 產生它，
所以內容不可能與實際掛載脫節：

```
/home/agent/AGENTS.md   ← 由 catalog 產生
/home/agent/CLAUDE.md   ← 同一份
/home/agent/GEMINI.md   ← 同一份
```

每個 agent 拿到的內容**只有自己那一側**：

| 角色 | 被告知 | 不被告知 |
| --- | --- | --- |
| leader | 派工流程、完整驗收閘門清單 | — |
| worker | 只接受 leader 提及、回報對象、自己的職責 | 驗收閘門的判準 |

worker 若知道 leader 的驗收標準，就能對「它本來就不該接受的指令」進行推理。
所以那份清單只給 leader。

### 驗證流程確實可運作

```bash
bash scripts/verify-task-flow.sh
```

這不是示範腳本，是**驗證腳本**——26 項斷言，包含每一個「應該失敗」的情況：
worker 不能自行建立任務、不可派工給 leader、空證據不可驗收、CI 非 success
不可驗收、交付分支不符不可驗收、無 merge 證據不可結案、closed 是終態。
閘門若被放寬，這裡會失敗。

> ⚠️ **Discord 自動派工尚未實作**（追蹤編號 A007）。
> 上述驗證涵蓋的是任務生命週期與准入政策，
> 實際把訊息送進 worker 頻道目前仍需由 leader 操作員手動進行。
...
```bash
# 先看控制平面能從 checkout 讀出什麼（唯讀，不改任何狀態）
python3 -m oab_control.cli task-collect <task-id> \
  --catalog config/catalog.yaml --tasks-dir .oab-control/tasks --json

# 驗收時讓它自動填入，不要手動打 commit_sha
python3 -m oab_control.cli task-transition <task-id> accepted \
  --tasks-dir .oab-control/tasks \
  --evidence-file attested.json \
  --collect config/catalog.yaml --json
```

`attested.json` 只需要放**無法查證**的欄位（測試結果、review、CI、
人類授權、`push_ref`、`merge_request`）。`commit_sha`、`delivery_branch`、
`delivery_repository`、`delivery_owner` 由 `--collect` 讀取——
手動填入不一致的值會被拒絕。

收取會在這些情況 fail closed：checkout 不存在、分支與任務不符、
worktree 有未提交變更、分支相對 base 沒有任何 commit。

> `push` 由**操作員**執行，不是 agent。順序見
> [規格 §5.4](規格-agent-工作流程.md#54-誰執行-push)。
...
四個 agent 各自對應一個**私有**頻道（leader 是入口頻道，其餘三個是 worker 私有頻道）。

頻道必須由**人類建立**——四個 bot 都沒有 `MANAGE_CHANNELS`，
而讓 AI agent 的 bot 能改動伺服器結構，會是整個設計裡最大的一筆授權。
...
# 或明確指定既有頻道
python3 scripts/bind-discord-channels.py --map \
  leader=<id>,researcher=<id>,developer=<id>,reviewer=<id>

# 只驗證不寫入
python3 scripts/bind-discord-channels.py --check
```

它做兩層檢查，都是**實際呼叫 API**而非推測權限覆寫
（權限可能來自角色，只看覆寫會誤判）：

1. 每個 bot 讀得到分配給它的頻道
2. **leader 進得去每個 worker 的頻道** ← 派工的前提

第 2 項是最容易漏掉的：私有 worker 頻道通常只授權給該 worker，
leader 會拿到 403，於是**派工訊息永遠送不出去，而控制平面完全不會察覺**。

兩層都通過才寫入 `config/catalog-input.json`。

### Discord 端需要的權限配置

| 頻道 | 誰需要存取 |
| --- | --- |
| leader 入口頻道 | leader bot、你（人類） |
| 每個 worker 私有頻道 | 該 worker bot、**leader bot**、你 |

在 Discord：三個 worker 頻道各自 → 編輯頻道 → 權限 → 新增身分組
→ 加入 **`leader` 身分組**（leader bot 持有此角色）
→ 允許「檢視頻道」「傳送訊息」「讀取訊息紀錄」。

或用腳本（需要一個具 MANAGE_ROLES 的 bot token）：

```bash
OAB_ADMIN_BOT_TOKEN=<token> python3 scripts/grant-leader-channel-access.py
```

不帶 token 執行會列出精確的變更內容（頻道 ID、角色 ID、權限位元）。

> ⚠️ 換頻道會讓既有任務的 `reply_to` 與 catalog 不符，
> `status` 會標記 `catalog_binding: mismatch`。
> 正確處理是取消該任務並以新頻道重建（見「更新」章節）。
...
developer:
  runtime:
    ...
    image: ghcr.io/openabdev/openab:0.10.0-beta.3-claude
