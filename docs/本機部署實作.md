...
| 四個 agent 的 workspace（獨立 clone） | ✅ 48 個 checkout，2.3G |
...
| 驗證項目 | 結果 |
| --- | --- |
| 每個容器內可見的 workspace | ✅ **12 個**（4 agent × 12 = 48 個 hostPath 掛載） |
...
本機目前授權 12 個 repository grant，分屬三個 collection root：

| Collection root | repo 數 | 說明 |
| --- | --- | --- |
| `~/repositories` | 1 | WSL 本機的 repo |
...
flowchart LR
  ...
  subgraph HOST["WSL 本機（ext4）"]
    COL["~/repositories<br/>1 個 repo"]
    REM["~/git-remotes/<br/>12 個 bare mirror · 359M"]
    subgraph WT["~/oab-agent-worktrees/"]
      W["leader / researcher<br/>developer / reviewer<br/>各 12 個 checkout"]
    end
  end
  subgraph K3S["K3s · oab-agents"]
    P["4 個 Pod<br/>各 12 個 hostPath 掛載"]
  end
  GL["公司 GitLab"]
...
12 repo × 4 agent = 48 個 grant 區塊，手寫必然出錯。單一來源是
...
```
~/repositories/                    WSL 本機的 collection root
~/git-remotes/<collection>/*.git   12 個 bare mirror（359M）= delivery remote
~/oab-agent-worktrees/<agent>/<collection>/<repo>
                                   48 個獨立 checkout（2.3G）
