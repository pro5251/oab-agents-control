...
runtime: unknown   ← 見「已知限制」的 pods 權限說明
```
...
# 直接在 developer 容器內列出被掛載的 workspace
kubectl -n oab-agents exec deploy/developer -- ls -la /workspaces/developer/openab-council-relay

# 確認是完整 git checkout，且在正確的 task 分支上
kubectl -n oab-agents exec deploy/developer -- sh -c \
  'cd /workspaces/developer/openab-council-relay && git log --oneline -3 && git branch --show-current'

# 確認唯讀 grant 真的唯讀（預期失敗）
kubectl -n oab-agents exec deploy/reviewer -- \
  sh -c 'touch /workspaces/reviewer/openab-council-relay/SHOULD_FAIL' \
  && echo "❌ 唯讀掛載失效" || echo "✅ 唯讀掛載生效"

# 確認 developer 可寫
kubectl -n oab-agents exec deploy/developer -- \
  sh -c 'touch /workspaces/developer/openab-council-relay/.write-test && rm /workspaces/developer/openab-council-relay/.write-test' \
  && echo "✅ developer 可寫"
```
...
| **`status` 的 runtime 觀測** | 永遠回報 `unknown` | 見下方 |

### `status --environment` 無法觀測 runtime

`ControlOperations.status()` 用 **deployer** 憑證執行 `kubectl get pods`，
但 deployer 的 Role 沒有 `pods` 權限（實測 `kubectl auth can-i get pods` → `no`）。

所以 `status` 的 `runtime` 欄位會一直是
`{"state": "unknown", "reason": "K3s observation unavailable: ..."}`。
本機狀態（workspace、任務）仍然正確回報，只有叢集內的 pod 健康度看不到。

要修正需要二擇一：在 deployer Role 加上 `pods` 的 `get`/`list`/`watch`
（唯讀，不影響 Secret 邊界），或改用第三個唯讀觀測身分。
目前直接用 `kubectl -n oab-agents get pods`（admin kubeconfig）即可。

### Image 變體的限制

`renderer.py` 把 config.toml 的 `working_dir` **寫死成 `/home/agent`**，
所以目前只相容 home 為 `/home/agent` 的 image 變體：

| 變體 | user | home | 相容 |
| --- | --- | --- | --- |
| `0.9.0-beta.9-native` | agent | `/home/agent` | ✅ 目前使用 |
| `latest`（kiro） | agent | `/home/agent` | ✅ |
| `0.9.0-beta.9-claude` | node | `/home/node` | ❌ |
| `0.9.0-beta.9-gemini` | node | `/home/node` | ❌ |
| `0.9.0-beta.9-opencode` | node | `/home/node` | ❌ |

要改用 claude／gemini／opencode 變體，需要讓 `working_dir` 變成可設定
（`render_openab_values` 目前沒有把 `render_config_toml` 的 `working_dir`
參數接出來）。
